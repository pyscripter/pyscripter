...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: https://jvcl.svn.sourceforge.net/svnroot/jvcl/branches/JVCL3_33_PREPARATION/run/JvAppStorage.pas $';
  ...
  begin
    ...
    case Location of
      flCustom:
        Result := FileName;
      flTemp:
        Result := IncludeTrailingPathDelimiter(GetWindowsTempFolder) + NameOnly;
      flWindows:
        Result := IncludeTrailingPathDelimiter(GetWindowsFolder) + NameOnly;
      flExeFile:
        Result := ExtractFilePath(Application.ExeName) + NameOnly;
      flUserFolder:
        Result := IncludeTrailingPathDelimiter(GetAppdataFolder) + RelPathName;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    case Location of
      ...
      flExeFile:
        FFullFileName := IncludeTrailingPathDelimiter(ExtractFilePath(ParamStr(0))) + NameOnly;
      {$IFDEF MSWINDOWS}
      flTemp:
        FFullFileName := IncludeTrailingPathDelimiter(GetWindowsTempFolder) + NameOnly;
      flWindows:
        {$IFDEF CLR}
        FFullFileName := '';
        {$ELSE}
        FFullFileName := IncludeTrailingPathDelimiter(GetWindowsFolder) + NameOnly;
        {$ENDIF ~CLR}
      flUserFolder:
        FFullFileName := IncludeTrailingPathDelimiter(GetAppdataFolder) + RelPathName;
      {$ENDIF MSWINDOWS}
      {$IFDEF UNIX}
      flTemp:
        FFullFileName := IncludeTrailingPathDelimiter(GetTempDir) + NameOnly;
      flUserFolder:
        FFullFileName := IncludeTrailingPathDelimiter(GetEnvironmentVariable('HOME')) + RelPathName;
      {$ENDIF UNIX}
    end;
  end;
