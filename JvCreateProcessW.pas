...
type
  ...
  public
    constructor Create;
    property StartupInfo: TStartupInfoW read GetStartupInfo;
  published
    property Desktop: WideString read FDesktop write FDesktop;
  ...
  TJvCreateProcessW = class(TJvComponent)
  private
  ...
  published
    ...
    property CurrentDirectory: WideString read FCurrentDirectory write FCurrentDirectory;
...
constructor TJvBaseReader.Create(ACreateProcess: TJvCreateProcessW);
begin
  inherited Create;
...
constructor TJvCreateProcessW.Create(AOwner: TComponent);
begin
...
destructor TJvCreateProcessW.Destroy;
begin
  TerminateWaitThread;
  FErrorReader.Free;
  FInputReader.Free;
...
function TJvCreateProcessW.GetConsoleOutput: TStrings;
begin
  Result := FInputReader.ConsoleOutput;
end;

function TJvCreateProcessW.GetEnvironment: TStrings;
begin
  Result := FEnvironment;
end;

function TJvCreateProcessW.GetHandle: THandle;
begin
  if FHandle = 0 then
...
function TJvCreateProcessW.GetOnErrorRawRead: TJvCPSRawReadEvent;
begin
  Result := FErrorReader.OnRawRead;
end;

function TJvCreateProcessW.GetOnErrorRead: TJvCPSReadEvent;
begin
  Result := FErrorReader.OnRead;
end;

function TJvCreateProcessW.GetOnRawRead: TJvCPSRawReadEvent;
begin
  Result := FInputReader.OnRawRead;
end;

function TJvCreateProcessW.GetOnRead: TJvCPSReadEvent;
begin
  Result := FInputReader.OnRead;
end;

procedure TJvCreateProcessW.GotoReadyState;
begin
  CheckNotWaiting;
  FState := psReady;
  CloseProcessHandles;
...
procedure TJvCreateProcessW.GotoRunningState;
begin
  CheckReady;
  FState := psRunning;
  CloseProcessHandles;
...
procedure TJvCreateProcessW.GotoWaitState(const AThreadCount: Integer);
begin
  CheckReady;
  FState := psWaiting;
...
procedure TJvCreateProcessW.HandleReadEvent(Sender: TObject);
begin
  TJvReader(Sender).HandleReadEvent;
end;

procedure TJvCreateProcessW.Run;
const
...
begin
  ...
  if Length(FApplicationName) > 0 then begin
    FApplicationName := Trim(FApplicationName);
    AppName := PWideChar(FApplicationName);
  end else
    AppName := nil;

  if Length(FCurrentDirectory) > 0 then begin
    FCurrentDirectory := Trim(FCurrentDirectory);
    CurrDir := PWideChar(FCurrentDirectory);
  end else
    CurrDir := nil;

  if Length(FCommandLine) > 0 then begin
    FCommandLine := Trim(FCommandLine);
    CommandLine := PWideChar(FCommandLine);
  end else
    CommandLine := nil;
  ...
  try
    ...
    if not CreateProcessW(AppName, CommandLine, nil, nil, coRedirect in ConsoleOptions,
...
procedure TJvCreateProcessW.SetEnvironment(const Value: TStrings);
begin
  FEnvironment.Assign(Value);
end;

procedure TJvCreateProcessW.SetOnErrorRawRead(
  const Value: TJvCPSRawReadEvent);
begin
  FErrorReader.OnRawRead := Value;
end;

procedure TJvCreateProcessW.SetOnErrorRead(const Value: TJvCPSReadEvent);
begin
  FErrorReader.OnRead := Value;
end;

procedure TJvCreateProcessW.SetOnRawRead(const Value: TJvCPSRawReadEvent);
begin
  FInputReader.OnRawRead := Value;
end;

procedure TJvCreateProcessW.SetOnRead(const Value: TJvCPSReadEvent);
begin
  FInputReader.OnRead := Value;
end;

procedure TJvCreateProcessW.SetStartupInfo(Value: TJvCPSStartupInfo);
begin
  FStartupInfo.Assign(Value);
end;

procedure TJvCreateProcessW.SetWaitForTerminate(const Value: Boolean);
begin
  GotoReadyState;
  FWaitForTerminate := Value;
end;

procedure TJvCreateProcessW.StopWaiting;
begin
  TerminateWaitThread;
end;

procedure TJvCreateProcessW.Terminate;
begin
  CheckRunning;
  InternalTerminateProcess(FProcessInfo.dwProcessId);
end;

procedure TJvCreateProcessW.TerminateWaitThread;
begin
...
procedure TJvCreateProcessW.WaitThreadTerminated(Sender: TObject);
begin
...
procedure TJvCreateProcessW.WndProc(var Msg: TMessage);
begin
  try
    with Msg do
      case Msg of
