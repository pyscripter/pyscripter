...
type
  ...
  TOffice2007Scheme = (osBlue, osMetallic, osGreen, osUnknown);

  TTBXOffice2007Theme = class(TTBXTheme)
  private
  ...
  begin
    ...
    if ItemInfo.Selected then
     ...
     //chy 2007.01.15 PaintGradient(DC, R, GetBtnColor(ItemInfo, ipBody), GetBtnColor(ItemInfo, ipBody, True));
      FillGradient(Canvas,R, GetBtnColor(ItemInfo, ipBody,true), clwhite,tGTopBottom);
    InflateRect(R, 1, 1);
    Canvas.Pen.Color :=  $00C5C5C5;//PopupFrameColor; //ToolbarFrameColor1;
    Canvas.Brush.Style := bsClear;
    Canvas.RoundRect(R.Left, R.Top, R.Right, R.Bottom, 4, 4);
  end;
end;
...
begin
  ...
  if (ItemInfo.ViewType and PVT_TOOLBOX) <> PVT_TOOLBOX then with Canvas do
    begin
      ...
      if GetOffice2007Scheme = osBlue then
        begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if GetOffice2007Scheme = osBlue then  begin
...
const // $FFC69A
  Office2007Colors: array[osBlue..osGreen, 0..27] of TColor = (
    ...
    $6F4B4B, $99797A, $8CD5FF, $69BDFF, $4E91FE, $8ED3FF, $CCF4FF, $91D0FF,
...
begin
  ...
  begin
    ...
    Scheme := GetOffice2007Scheme;
    if Scheme <> osUnknown then
    begin
      DockColor := Office2007Colors[Scheme, 0];

      ToolbarColor1 := Office2007Colors[Scheme, 1];
      ToolbarColor2 := Office2007Colors[Scheme, 2];
      ToolbarFrameColor1 := Office2007Colors[Scheme, 3];
      ToolbarFrameColor2 := Office2007Colors[Scheme, 4];
      SeparatorColor1 := Office2007Colors[Scheme, 5];
      SeparatorColor2 := Office2007Colors[Scheme, 6];
      DragHandleColor1 := Office2007Colors[Scheme, 7];
      DragHandleColor2 := Office2007Colors[Scheme, 8];

      EmbeddedColor := Office2007Colors[Scheme, 9];
      EmbeddedFrameColor := Office2007Colors[Scheme, 10];
      EmbeddedDisabledColor := Office2007Colors[Scheme, 11];

      PopupColor := Office2007Colors[Scheme, 12];
      PopupFrameColor := Office2007Colors[Scheme, 13];

      DockPanelColor := ToolbarColor1;

      EnabledText := Office2007Colors[Scheme, 14];
      DisabledText := Office2007Colors[Scheme, 15];
      MenuItemFrame := Office2007Colors[Scheme, 16];

      WinFrameColors[wfpBorder] := Office2007Colors[Scheme, 17];
      ...
      BtnBodyColors[bisDisabled, False] := clNone;
      BtnBodyColors[bisDisabled, True] := clNone;
      BtnBodyColors[bisSelected, False] := Office2007Colors[Scheme, 18];
      BtnBodyColors[bisSelected, True] := Office2007Colors[Scheme, 19];
      BtnBodyColors[bisPressed, False] := Office2007Colors[Scheme, 20];
      BtnBodyColors[bisPressed, True] := Office2007Colors[Scheme, 21];
      BtnBodyColors[bisHot, False] := Office2007Colors[Scheme, 22];
      BtnBodyColors[bisHot, True] := Office2007Colors[Scheme, 23];
      ...
      BtnBodyColors[bisPopupParent, False] := Office2007Colors[Scheme, 24];
      BtnBodyColors[bisPopupParent, True] := Office2007Colors[Scheme, 25];
      ...
      MenuItemColors[misDisabled, ipFrame] := clNone;
      MenuItemColors[misHot, ipBody] := Office2007Colors[Scheme, 26];
      ...
      FMDIAreaColor := Office2007Colors[Scheme, 27];
    end
    else
    begin
      DockColor := clBtnFace;
    ...
    IrregularGradientValue := Office2007IrregularGradientValues[Scheme];
...
procedure TTBXOffice2007Theme.TBXSysCommand(var Message: TMessage);
begin
