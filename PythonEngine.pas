...
{$ELSE}
  ...
  {$ENDIF}
{$ENDIF}
  MethodCallBack, TinyWideStrings;
...
{$ENDIF}
  ...
  protected
    ...
    FMaxLines        : Integer;
    FLine_Buffer     : WideString;
    FLinesPerThread  : TWideStringList;
    FLock            : TCriticalSection;
    FQueue           : TWideStringList;
    ...
    procedure Lock;
    procedure Unlock;
    procedure AddWrite( const str : WideString );
    function  GetCurrentThreadLine : WideString;
...
{$ENDIF}
  ...
  public
    ...
    procedure Write( const str : WideString );
    procedure WriteLine( const str : WideString );
  published
...
begin
  inherited;
  FMaxLines      := kMaxLines;
  FQueue         := TWideStringList.Create;
  FDelayWrites   := False;
  FMaxLineLength := kMaxLineLength;
  FLinesPerThread:= TWideStringList.Create;
...
procedure TPythonInputOutput.Write( const str : WideString );
  procedure DropLine;
  begin
{$IFDEF MSWINDOWS}
    if DelayWrites then
...
procedure TPythonInputOutput.WriteLine( const str : WideString );
begin
  Write( str+#10 );
end;

procedure TPythonInputOutput.AddWrite( const str : WideString );
begin
  FQueue.Add( str );
...
function  TPythonInputOutput.GetCurrentThreadLine : WideString;
begin
...
{$ENDIF}
{$IFDEF PYTHON22_OR_HIGHER}
  //PyExc_OverflowWarning      := Import('PyExc_OverflowWarning');
...
//  {$IFNDEF PYTHON25_OR_HIGHER}
//    PySymtableEntry_Type     := Import('PySymtableEntry_Type');
//  {$ENDIF}
...
{$ENDIF}
//{$IFDEF PYTHON22_OR_HIGHER}
//      else if (PyErr_GivenExceptionMatches(err_type, PyExc_OverflowWarning^) <> 0) then
//        raise Define( EPyOverflowWarning.Create(''), s_type, s_value )
//{$ENDIF}
{$IFDEF PYTHON23_OR_HIGHER}
